...
import static todos.TodosOuterClass.*;
...
//        return Multi.createFrom().ticks().every(Duration.ofSeconds(1)).onItem().transform(todo -> Todo.newBuilder().setId(idSequence.get()).setTodoState(State.NEW).setTitle("creado").setDescription("una descripcion").build());
    ...
    public Uni<TodosOuterClass.Void> markDone(Todo request) {
        Todo todo = Todo.newBuilder(request).setTodoState(State.DONE).build();
